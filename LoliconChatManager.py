...
version = f"v{__version__[0]}.{__version__[1]}.{__version__[2]}release"
ver = f"<i>LoliconChatManager{version}</i>"
...
class LoliconChatManagerAPI:
    ...
    async def _wss(self):
        async with websockets.connect(
            f"wss://LoliconChatManager.hikariatama.ru/ws/{self.module.get('token')}"
        ) as wss:
            init = json.loads(await wss.recv())

            logger.debug(f"LoliconChatManager connection debug info {init}")
            ...
            logger.debug("LoliconChatManager connected")
            self._show_warning = True
    ...
    async def _connect(self):
        while True:
            try:
                ...
                logger.debug("LoliconChatManager disconnection traceback", exc_info=True)
                ...
                if self._show_warning:
                    logger.debug("LoliconChatManager disconnected, retry in 5 sec")
    ...
    async def nsfw(self, photo: bytes) -> str:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.request(
                "POST",
                "https://LoliconChatManager.hikariatama.ru/check_nsfw",
...
@loader.tds
class LoliconChatManagerMod(loader.Module):
    """
    ...
    strings_ru = {
        ...
        "error": "😵 <b>Произошла ошибка LoliconChatManager</b>",
        "args": (
    ...
    def error_handler(function) -> FunctionType:
        ...
        async def wrapped(*args, **kwargs):
            try:
                ...
                logger.exception("Exception caught in LoliconChatManager")
    ...
    async def get_config(self, chat: typing.Union[str, int]) -> tuple:
        ...
        answer_message = (
            f"🪆 <b>LoliconChatManager protection</b>\n<b>{get_full_name(cinfo)}</b>\n\n"
        )
        ...
        for _ in range(5):
            try:
                m = await self.inline.form(
                    ...
                    photo=f"https://LoliconChatManager.hikariatama.ru/captcha/{valid}",
                    reply_markup=markup,
